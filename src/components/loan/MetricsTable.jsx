...
export default function MetricsTable({ form, result }) {
  ...
  const rows = [
    ...
    { metric: "DTI (Current)", value: `${(result.dti * 100).toFixed(1)}%`, gate: <GateBadge status={result.gates.dti} /> },
    { metric: "DTI (Total with New EMI)", value: `${(result.totalDTI * 100).toFixed(1)}%` },
