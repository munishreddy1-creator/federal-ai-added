...
const kpiConfig = [
  { key: "emi", label: "Monthly EMI", icon: IndianRupee, format: (v) => fmt(v), color: "text-blue-700" },
  ...
  { key: "surplus", label: "Monthly Surplus", icon: Wallet, format: (v) => fmt(v), color: "text-green-600" },
...
export default function KPICards({ result }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
      {kpiConfig.map(({ key, label, icon: Icon, format, color }, idx) => {
